...
def handleJoyStickAsArrowKeys(x, y, z):      #note that the x and y directions are swapped due to the way I orient my thumbstick
    if x == 3:          #0 is up on joystick
        ...
        keyUp('up')
        keyDown('up')
    elif x == 1:        #2 is down on joystick
        keyDown('up')   #add up key to keyDown (argument)
        keyUp('down')   #add down key to keyUp (argument), as you can't press up and down together
        keyUp('up')
    ...
    if y == 5:          #5 is right on joystick
        keyDown('right')
        keyUp('left')
    elif y == 4:          #4 is right on joystick
        keyDown('right')
        keyUp('left')
        keyUp('right')
        ...
        keyDown('left')
        keyUp('right')
    elif y == 1:        #1 is left on joystick
        keyDown('left')
        keyUp('right')
        keyUp('left')
    else:               #2 is neutral on joystick
        keyUp('left')
        keyUp('right')
